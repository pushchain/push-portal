...
import {
  ...
  getActivityStatus,
  getDayNumber,
} from "../utils/resolveRecentActivityStatus";
...
const useDailyRewards = () => {
  ...
  const dailyActivities = activityList.filter((activity) =>
    activity.index.startsWith(`daily-activity`),
  );
  ...
  const activityTitles = dailyRewardsActivities?.map(
    (activity) => activity.activityType,
  );

  // Check if dailyRewardsActivities is available and all activityTitles are defined
  const areActivitiesDefined =
    dailyRewardsActivities &&
    activityTitles?.every((title) => title !== undefined);

  const { data: sendRecentActivities, refetch: refetchSendActivities } =
    useGetRewardActivityStatus(
      {
  ...
  const handleCheckIn = useCallback(() => {
    ...
    const { isEmpty, firstEmptyActivity, latestActivityKey } =
      getActivityStatus(dataActivity);

    const targetActivity = isEmpty
      ? dailyRewardsActivities?.find(
          (activity) => activity.activityType === firstEmptyActivity,
        )
      : dailyRewardsActivities?.find(
          (activity) => activity.activityType === latestActivityKey,
        );

    const newDay = isEmpty ? 1 : getDayNumber(targetActivity) + 1;
    const newDayData = dailyRewardsActivities?.find(
      (activity) =>
        activity.activityType === `daily_check_in_7_days_day${newDay}`,
    );

    if (latestActivityKey && !isEmpty) {
      const latestActivity = dataActivity?.[
        latestActivityKey
      ] as RewardActivityStatus;
