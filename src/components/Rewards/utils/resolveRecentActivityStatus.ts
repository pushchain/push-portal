...
export const getDayNumber = (activity: any) => {
  const dayMatch =
    activity?.activityTitle.match(/Day (\d+)/) ||
    activity?.activityType.match(/day(\d+)/);
...
export const getActivityStatus = (
  ...
  for (const [key, value] of Object.entries(activities)) {
    if (Object.keys(value).length === 0) {
      ...
      isEmpty = false;
      nonEmptyActivities.push({
        key,
        updatedAt: processDateString(value?.updatedAt),
      });
    }
  }
  ...
  const closestActivity =
    nonEmptyActivities.length > 0
      ? findClosest(nonEmptyActivities, (item) => item.updatedAt)
      : null;
