...
const UnlockProfile = ({
  ...
  const handleChatprofileUnlock = useCallback(async () => {
    const user = await handleConnectWalletAndEnableProfile({
      remember: rememberMe,
      wallet: account,
    });
  ...
  const isMobile = useMediaQuery(deviceSizes.mobileL);
        ...
        >
          ...
          >
            ...
            <StepsLeftDesign
              ...
              color={
                activeStatus.status !== PROFILESTATE.CONNECT_WALLET
                  ? theme.btn.primaryColor
                  : theme.btn.disabledColor
              }
            >
              2
            </StepsLeftDesign>
