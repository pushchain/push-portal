import { C as l, N as k, S as g, g as w, a as f, b as u, B as A, A as N } from "./core-DjH87310.js";
import "./main-RE-IXpVn.js";
class m {
  constructor(e) {
    this.getNonce = e.getNonce;
  }
  async createMessage(e) {
    const t = { accountAddress: e.accountAddress, chainId: e.chainId, version: "1", domain: typeof document > "u" ? "Unknown Domain" : document.location.host, uri: typeof document > "u" ? "Unknown URI" : document.location.href, resources: this.resources, nonce: await this.getNonce(e), issuedAt: this.stringifyDate(/* @__PURE__ */ new Date()), statement: void 0, expirationTime: void 0, notBefore: void 0 };
    return Object.assign(t, { toString: () => this.stringify(t) });
  }
  stringify(e) {
    var _a;
    const t = this.getNetworkName(e.chainId);
    return [`${e.domain} wants you to sign in with your ${t} account:`, e.accountAddress, e.statement ? `
${e.statement}
` : "", `URI: ${e.uri}`, `Version: ${e.version}`, `Chain ID: ${e.chainId}`, `Nonce: ${e.nonce}`, e.issuedAt && `Issued At: ${e.issuedAt}`, e.expirationTime && `Expiration Time: ${e.expirationTime}`, e.notBefore && `Not Before: ${e.notBefore}`, e.requestId && `Request ID: ${e.requestId}`, ((_a = e.resources) == null ? void 0 : _a.length) && e.resources.reduce((n, s) => `${n}
...
class E {
  constructor(e = {}) {
    this.otpUuid = null, this.listeners = { sessionChanged: [] }, this.localAuthStorageKey = e.localAuthStorageKey || g.SIWX_AUTH_TOKEN, this.localNonceStorageKey = e.localNonceStorageKey || g.SIWX_NONCE_TOKEN, this.required = e.required ?? true, this.messenger = new m({ getNonce: this.getNonce.bind(this) });
  }
  async createMessage(e) {
    return this.messenger.createMessage(e);
  }
  async addSession(e) {
    const t = await this.request({ method: "POST", key: "authenticate", body: { data: e.data, message: e.message, signature: e.signature, clientId: this.getClientId(), walletInfo: this.getWalletInfo() }, headers: ["nonce", "otp"] });
  ...
  async getSessions(e, t) {
    try {
      ...
      const n = await this.request({ method: "GET", key: "me", query: {}, headers: ["auth"] });
      if (!n) return [];
      const s = n.address.toLowerCase() === t.toLowerCase(), o = n.caip2Network === e;
      if (!s || !o) return [];
      const r = { data: { accountAddress: n.address, chainId: n.caip2Network }, message: "", signature: "" };
  ...
  async setSessions(e) {
    if (e.length === 0) this.clearStorageTokens();
      ...
      const t = e.find((n) => {
        var _a;
        return n.data.chainId === ((_a = w()) == null ? void 0 : _a.caipNetworkId);
      }) || e[0];
  ...
  async getSessionAccount() {
    ...
    return this.request({ method: "GET", key: "me", body: void 0, query: { includeAppKitAccount: true }, headers: ["auth"] });
  }
  async setSessionAccountMetadata(e = null) {
    if (!this.getStorageToken(this.localAuthStorageKey)) throw new Error("Not authenticated");
    return this.request({ method: "PUT", key: "account-metadata", body: { metadata: e }, headers: ["auth"] });
  }
  on(e, t) {
  ...
  async requestEmailOtp({ email: e, account: t }) {
    const n = await this.request({ method: "POST", key: "otp", body: { email: e, account: t } });
    return this.otpUuid = n.uuid, this.messenger.resources = [`email:${e}`], n;
  }
  confirmEmailOtp({ code: e }) {
    return this.request({ method: "PUT", key: "otp", body: { code: e }, headers: ["otp"] });
  }
  async request({ method: e, key: t, query: n, body: s, headers: o }) {
    ...
    const c = await fetch(a, { method: e, body: s ? JSON.stringify(s) : void 0, headers: Array.isArray(o) ? o.reduce((i, h) => {
      switch (h) {
        case "nonce":
    ...
    return ((_a = c.headers.get("content-type")) == null ? void 0 : _a.includes("application/json")) ? c.json() : null;
  }
  getStorageToken(e) {
    return u.getItem(e);
  }
  ...
  async getNonce() {
    const { nonce: e, token: t } = await this.request({ method: "GET", key: "nonce" });
  ...
  getWalletInfo() {
    var _a;
    const e = (_a = l.getAccountData()) == null ? void 0 : _a.connectedWalletInfo;
    if (!e) return;
